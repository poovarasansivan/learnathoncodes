...
function Body() {
    ...
    useEffect(() => {
        axios({
            ...
            .then((res1) => {
                setMyEventsData(res1.data); // Assuming `res1.data` is an array of events
            })
            .catch((err) => {
                console.log(err);
            });
        ...
        <div className="m-2 md:m-10 mt-20 p-2 md:p-6 bg-white rounded-3xl">
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10 dark:text-gray-200 dark:bg-secondary-dark-bg'>
                {MyeventsData.length > 0 ? (
                    MyeventsData.map((item, index) => (
                        <div key={index} className="bg-gray-50 p-5 rounded-2xl max-h-max-content relative cursor-pointer shadow-md" onClick={navigateToMyDetails}>
                            <div className="px-1 py-4 bg-gray-50">
                                ...
                                <p className="text-gray-700 mt-2 text-base">Events Date: {item.event_date}</p>
